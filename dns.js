...
let requestCount = 0; // ✅ Counter request ditambahkan
...
// ✅ Search form dengan logika captcha setelah request ke-3
document.getElementById('searchForm').addEventListener('submit', async function(e) {
    e.preventDefault();
    
    requestCount++; // Hitung jumlah request
    const ipAddress = document.getElementById('ipAddress').value.trim();

    // ✅ CAPTCHA hanya diminta mulai request ke-3
    let recaptchaResponse = '';
    if (requestCount >= 3) {
        recaptchaResponse = grecaptcha.getResponse();
        if (!recaptchaResponse) {
            showNotification('Silakan selesaikan reCAPTCHA dulu.', 'error');
            return;
        }
    }
    ...
    try {
        // ✅ CAPTCHA hanya dikirim jika request >= 3x
        const captchaParam = requestCount >= 3 
            ? `&g-recaptcha-response=${encodeURIComponent(recaptchaResponse)}`
            : '';

        const response = await fetch(`api.php?ip_address=${encodeURIComponent(ipAddress)}${captchaParam}`);
...
function redirectToTelegram(planType) {
    const telegramUsername = 'xiera0069';
